...
app.get('/', async (req, res) => {
    ...
    let up
    try {
        up = await request({ url: upstream_uri, headers: headers })
    } catch (error) {
        up = error
    }
...
function createIssues(req, res) {
    ...
    // Cascade trace headers and the fail and crash flags to upstream services
    // The trace headers are the only "Istio Specifc" thing your app needs to do!
